...
List hrbcpp(arma::vec y, arma::vec x, arma::vec K, arma::vec L, arma::vec res, double c, int p, int q, double h, double b) { 
  ...
  arma::mat lbc0 = lus0 - pow(rho,p+2)*(e0.t()*iGp)*Lp1*ep1.t()*iGq*(dL*Rq).t();
  ...
  arma::mat q7(1,1,arma::fill::zeros);
  ...
  for (int i = 0 ; i < N ; i++) {
    ...
    arma::mat lbc1 = lus1 - pow(rho,p+2)*(T1 + T2 + T3);
    
    q2 += lbc1*lbc0(i)*pow(lbc0(i)*res(i),2)*vx(i);
    
    q3 += pow(lbc0(i),4)*(pow(res(i),4)-pow(vx(i),2));
    
    q4 += pow(lbc0(i),2)*(Rqi*iGq*L(i)*Rqi.t())*pow(res(i),2);

    q5a += pow(lbc0(i),3)*(Rqi*iGq)*pow(res(i),3);
    q5b += L(i)*Rqi.t()*lbc0(i)*pow(res(i),2);
    
    q8 += pow(lbc0(i),4)*(pow(res(i),4));
    ...
    for (int j = 0 ; j < N ; j++) { 
      if (j != i) {
        ...
        for (int k = 0 ; k < N ; k++) { 
          if (k != j) {
            arma::mat Rqk = Rq.row(k);
            q7 += (pow(lbc0(j),2)*Rqj*iGq*L(i)*Rqi.t()) * (lbc0(i)*Rqj*iGq*L(k)*Rqk.t()) * lbc0(k)*pow(res(i)*res(k),2);
          }
        }
      }
    }
  ...
  arma::mat Eq7  = q7/(N*(N-1)*(N-2)*pow(h,3));
  arma::mat Eq8  = q8/(N*h);
  arma::mat Eq9  = q9/(N*h);
